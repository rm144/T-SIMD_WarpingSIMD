...
public:
  ...
  WarpingExample(int w, int nAlpha, int nPsi)
    : // 31. Jul 17 (rm): BUGFIX: wsc() -> wsc(0)
      spsComp(0), partialSearcherMinWarping(0), wsc(0), nScalePlanes(9),
      maxScaleFactor(2.0), maxThresholdMinWarping(2.5), rhoMinMinWarping(0.0),
      rhoMaxMinWarping(100.0), rhoMinWarping(0.0), rhoMaxWarping(1.0),
      nRhoWarping(20), doubleSearch(1), interpolation(0), psiFraction(0.3)
  {
    ...
    partialSearcherMinWarping =
      new MinWarpingPartial_XPAY<SPSType, MatchType, SIMD_WIDTH, SIMD_ALIGN>();
  ...
  void init(int w, int nAlpha, int nPsi)
  {
    // compound
    if (wsc) delete wsc;
           ...
           double &alphaMin, double &psiMin, double &dMin)
  {
    ...
    wsc->minWarping.compassAcceleration(*partialSearcherMinWarping,
                                        doubleSearch, psiFraction);
...
int main()
{
  ...
  assert((fscanf(f, "%d %d %d %d %d %lf %lf", &w, &xmin, &xmax, &ymin, &ymax,
                 &vertRes, &horizon) == 7));
  fprintf(stderr, "w = %d, x: %d %d, y: %d %d, vertRes = %g, horizon = %g\n", w,
          xmin, xmax, ymin, ymax, vertRes, horizon);
  fclose(f);
  ...
  // run warping
  // 27. Feb 18 (rm): vert.res. and hor. taken from images
  we.run(ss, cv, pixelScale, POSTSCALE, alphaMin, psiMin, dMin);
  ...
  printf("betaMin = %g, betaTrue = %g, betaAE = %g\n", betaMin, betaTrue,
         acos(cos(betaMin - betaTrue)));
  printf("psiMin = %g, psiTrue = %g, psiAE = %g\n", psiMin, psiTrue,
         acos(cos(psiMin - psiTrue)));
  return 0;
}
