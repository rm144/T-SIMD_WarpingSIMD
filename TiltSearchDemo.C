...
enum Strategy : int { GRID = 0, PATTERN = 1, NELDER_MEAD = 2 };
...
auto make_search(Warping &warping, Log &&log = Log {})
{
  ...
  case GRID: {
    ...
    return tilt::Search {tilt::RollPitch<Radian> {}, tilt_solution,
                         search::maybe_log(searcher, log), warping};
  }
    // Construct pattern search
  case PATTERN: {
    // Hardcoded initial search pattern
    auto searcher =
      search::Pattern {std::tuple {0.0_rad, 0.0_rad}, 0.14, 0.02, 0.5};
    // Compose the warping tilt search
    return tilt::Search {tilt::RollPitch<Radian> {}, tilt_solution,
                         search::maybe_log(searcher, log), warping, limit};
  }
...
int main(int argc, char **argv)
{
  ...
  auto crop = [crop_upper, crop_lower](auto &image) {
    ...
    ns_simd::croppedView(image, (int) (crop_upper / vr),
                         (int) (crop_lower / vr), cropped);
  ...
  tilt::Search tilt_search = [&] {
    ...
    return logging ? make_search(warping, log) : make_search(warping);
  }();
  ...
  print_args(std::cout, argmin, min, warping.count(), warping.avg_time(),
             total_time);
  ...
  double tilt_error = acos(cos(dx_true - dx_est) * cos(dy_true) * cos(dy_est) +
                           sin(dy_true) * sin(dy_est));

  // Print  home direction angle estimates
  printf("# EST:   beta=%f, alpha=%f, psi=%f\n", beta_est, min.alpha, min.psi);
  // Print true (expected) home direction angles
  printf("# TRUE:  beta=%f, alpha=%f, psi=%f\n", beta_true, alpha_true,
         psi_true);
  // Print home direction errors
  printf("# ERROR: beta=%f, alpha=%f, psi=%f\n", beta_err, alpha_err, psi_err);
  // Print home direction angle estimates
  printf("# TILT EST:   delta_X=%f, delta_Y=%f\n", dx_est, dy_est);
  // Print true (expected) home direction angles
  printf("# TILT TRUE:  delta_X=%f, delta_Y=%f\n", dx_true, dy_true);
  // Print home direction errors
  printf("# TILT ERROR: %f\n", tilt_error);
  // Print time measurements
  printf("# TIME:  warping=%f, total=%ld\n", warping.avg_time(), total_time);
  // Print count of warping runs
  printf("# COUNT: warping=%zu\n", warping.count());
