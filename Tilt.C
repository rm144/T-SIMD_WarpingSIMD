...
int main(int argc, char **argv)
{
  ...
  Interpolation interpolation =
    (argc == 6) ? Interpolation {std::stoi(argv[5])} : Interpolation::NEAREST;
  ...
  auto crop = [crop_upper, crop_lower](auto &img) {
    ...
    ns_simd::croppedView(img, (int) (crop_upper / vr), (int) (crop_lower / vr),
                         cropped);
  ...
  auto tilt = tilt::RollPitch<Radian> {descriptor.delta_X, descriptor.delta_Y};
  ...
  // @formatter:off
  decltype(image) tmp;
  correction(image, tmp, tilt, interpolation);
  // @formatter:on
  // Stop measuring time
  ...
  if (!ns_simd::savePGM(image_path, crop(tmp))) {
    ...
    throw ns_simd::SIMDException {
      __FUNCTION__,
      "Failed to save the image file: " + image_path + ": " + strerror(errno)};
  }
