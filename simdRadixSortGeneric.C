...
struct _PayloadSortIndex<KEYTYPE, true>
{
  template <typename ELEMENTTYPE>
  static INLINE void
  set(ELEMENTTYPE &e, SortIndex i)
  {
    typedef typename
      KeyPayloadInfo<KEYTYPE, true>::UIntPayloadType PayloadType;
...
generateData(int repeats, SortIndex num, bool noDuplicates,
	     GENERATOR<KEYTYPE> &generator)
{
  typedef typename
...
template <bool WithPayload,
	  typename KEYTYPE>
typename KeyPayloadInfo<KEYTYPE, WithPayload>::UIntElementType *
generateData(int rndMode, unsigned int seed, int repeats,
	     SortIndex num, bool noDuplicates)
{
  RandWideUniform<KEYTYPE> randWideUniform(seed);
  RandNormal<KEYTYPE> randNormal(seed);
  switch (rndMode) {
  case 0:
    return generateData<WithPayload>
      (repeats, num, noDuplicates, randWideUniform); 
  case 1:
    return generateData<WithPayload>
      (repeats, num, noDuplicates, randNormal);
  default:
    fprintf(stderr, "invalid rndMode %d\n", rndMode);
    exit(-1);
  }
}
...
bool keysAreSorted(ELEMENTTYPE *d, SortIndex num)
{
  for (SortIndex i = 1; i < num; i++)
    ...
    if (compareKeys<KEYTYPE,1-UP>(d[i-1], d[i])) {
      return false;
    }
  return true;
}
...
{
  static bool
  payloadsAreOk(typename KeyPayloadInfo<KEYTYPE, false>::UIntElementType *,
		SortIndex)
  {
    return true;
  }
...
struct CheckPayloads<KEYTYPE, true>
{
  ...
  static bool
  payloadsAreOk(typename KeyPayloadInfo<KEYTYPE, true>::UIntElementType *d,
		SortIndex num)
  {
    ...
    typedef typename
      KeyPayloadInfo<KEYTYPE, true>::UIntPayloadType KeyAndPayloadType;
    ...
    for (SortIndex i = 0; i < num; i++) {
      ...
      if (uIntPayload > KeyAndPayloadType(num))
	return false;      
...
template <int> struct Config;

// 10. Feb 21 (rm): changed order

// ----- float -----
template <> struct Config<0> : _Config<float,false> {};
template <> struct Config<1> : _Config<float,true> {};
// ----- double -----
template <> struct Config<2> : _Config<double,false> {};
template <> struct Config<3> : _Config<double,true> {};
// ----- uint32_t -----
template <> struct Config<4> : _Config<uint32_t,false> {};
template <> struct Config<5> : _Config<uint32_t,true> {};
// ----- uint64_t -----
template <> struct Config<6> : _Config<uint64_t,false> {};
template <> struct Config<7> : _Config<uint64_t,true> {};
// ----- int32_t -----
template <> struct Config<8> : _Config<int32_t,false> {};
template <> struct Config<9> : _Config<int32_t,true> {};
// ----- int64_t -----
template <> struct Config<10> : _Config<int64_t,false> {};
template <> struct Config<11> : _Config<int64_t,true> {};
...
void printRadixThreadStats(RadixThreadStats *threadStats)
{
  ...
  for (size_t i = 0; i < threadStats->elements.size(); i++)
    printf("%zu\t%ld\t%ld\n",
	   i, threadStats->chunks[i], threadStats->elements[i]);
}
...
main(int argc, char *argv[])
{
  ...
  // print config
  printf
    ("RADIX_CONFIG: %d, WithPayload %d, sizeof: Data %zu KeyType %zu\n",
  ...
  Data *dAll = generateData<WithPayload,KeyType>
    (rndMode, seed, rep, num, nodup);
  // save first 100 elements
  std::ofstream rndSampleFile;
  rndSampleFile.open(std::string("rndSample") +
		     "_config" + std::to_string(RADIX_CONFIG) +
		     "_rndMode" + std::to_string(rndMode) +
		     ".dat");
  ...
  for (int r = 0; r < rep; r++, d += num) {
    ...
    if (meth == 0) {
      ...
      if (up) seqRadixSort<KeyType,1>(d, 0, num-1, thresh);
      else seqRadixSort<KeyType,0>(d, 0, num-1, thresh);
      ...
      if (up) seqRadixSort2<KeyType,1>(d, 0, num-1, thresh);
      else seqRadixSort2<KeyType,0>(d, 0, num-1, thresh);
      ...
      // ----- std::sort -----
      if (up) std::sort(d, d + num, compareKeys<KeyType, 1, Data>);
      else std::sort(d, d + num, compareKeys<KeyType, 0, Data>);
      ...
      if (up) simdRadixSortCompress<KeyType, 1>(d, 0, num-1, thresh);
      else simdRadixSortCompress<KeyType, 0>(d, 0, num-1, thresh);
      ...
      if (up) baselineRadixSort<KeyType, 1>(d, 0, num-1, thresh);
      else baselineRadixSort<KeyType, 0>(d, 0, num-1, thresh);
	...
	seqRadixSortThreads<KeyType,1>
	  (RadixThreadConfig(nthreads,
			     RadixThreadConfig::RADIX_FIFO_QUEUE,
			     0, 1.0),
	   threadStats,
	   d, 0, num-1, thresh);
      else
	...
	simdRadixSortCompressThreads<KeyType, 0>
      ...
      // ----- std::sort -----
      if (up) std::sort(std::execution::seq,
			d, d + num, compareKeys<KeyType, 1, Data>);
      else std::sort(std::execution::seq,
		     d, d + num, compareKeys<KeyType, 0, Data>);
      ...
      // ----- std::sort -----
      if (up) std::sort(std::execution::par,
			d, d + num, compareKeys<KeyType, 1, Data>);
      else std::sort(std::execution::par,
		     d, d + num, compareKeys<KeyType, 0, Data>);
      ...
      // ----- std::sort -----
      if (up) std::sort(std::execution::par_unseq,
			d, d + num, compareKeys<KeyType, 1, Data>);
      else std::sort(std::execution::par_unseq,
		     d, d + num, compareKeys<KeyType, 0, Data>);
      ...
      // ----- std::sort -----
      if (up) std::sort(std::execution::unseq,
			d, d + num, compareKeys<KeyType, 1, Data>);
      else std::sort(std::execution::unseq,
		     d, d + num, compareKeys<KeyType, 0, Data>);
