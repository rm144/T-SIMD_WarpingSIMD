...
std::vector<double> maxVec(const std::vector<double> &x,
                           const std::vector<double> &y)
{
  assert(x.size() == y.size());
  std::vector<double> z(x.size());
  for (size_t i = 0; i < z.size(); i++) z[i] = std::max(x[i], y[i]);
  return z;
}
...
void heapInfo(FILE *f)
{
  fprintf(f, "heap %d desc %lu mem %lu\n", mallinfo().uordblks,
          SharedSIMDPtrDesc::totalDescNo, SharedSIMDPtrBase::totalSize);
}
...
int getEnvInt(const char *name, const int defaultVal)
{
...
double getEnvDouble(const char *name, const double defaultVal)
{
  char *valstr = getenv(name);
...
char *getEnvString(const char *name, const char *defaultVal)
{
  char *valstr = getenv(name);
...
std::vector<double> getEnvDoubleVector(const char *name,
...
#define DOUBLE_VEC(NAME, DEFAULTVAL, DEFAULTSIZE)                              \
  std::vector<double> NAME = getEnvDoubleVector(#NAME, DEFAULTVAL, DEFAULTSIZE)
...
INT(firstPhase, WarpingSPSComputationSelector::firstPhaseNSAD2);
...
INT(searcher, WarpingSearcherSelector::minWarpingSearcher);
...
// XPAY: search loop order x, psi, alpha, y (typically fastest)
INT(searchMethodWarping, WarpingSearchSelector::searchMethodWarpingPARX);
...
int main(int argc, char *argv[])
{
  ...
  if (argc < 2) {
    fprintf(stderr, "warpingSIMDTest <action> [<base> <ss> <cv> <suffix> <bw>] "
                    "{<option>}*\n");
    exit(-1);
  }
  action = argv[1][0];
  ...
  if ((action != 'x') && (action != 'Z')) {
    ...
    sprintf(id, "%s_%s_%s_%s_%s%s", argv[2], argv[3], argv[4], argv[5], argv[6],
            expId);
    fprintf(finfo, "id = %s\n", id);
    ...
    assert((fscanf(f, "%d %d %d %d %d %lf %lf", &w, &xmin, &xmax, &ymin, &ymax,
                   &vertRes, &horizon) == 7));
    fprintf(finfo,
    ...
    for (int x = 0; x < nx; x++) {
      ...
      for (int y = 0; y < ny; y++) {
        ...
        if (rotateImages) {
          ...
          // 20. Sep 22 (Jonas Keller): use rand instead of drand48, since
          // drand48 is not available on Windows int rotIndex = (int) (drand48()
          // * w);
          // + 1.0f since drand48 returns values in [0,1)
          int rotIndex =
            (int) (((float) rand() / ((float) RAND_MAX + 1.0f)) * w);
    ...
    WarpingBundle<IMGTYPE, PROCTYPE, MEASTYPE, SPSTYPE, MATCHTYPE, COMPASSTYPE,
                  SW, SA>
      warping(w, nAlpha, nPsi, nScalePlanes, maxScaleFactor,
              maxThresholdMinWarping, rhoMinMinWarping, rhoMaxMinWarping,
              rhoMinWarping, rhoMaxWarping, nRhoWarping, firstPhase,
              searchMethodWarping, searchMethodMinWarping,
              HarrisParameter(harrisK, harrisBinomialFilterCount), sigmoidW,
              sigmoidW0);
    ...
    case 'p':
      ...
      // example: ./warpingSIMDTest p living3 day night Hh288sh 0.10
      {
        ...
        printf("SCALING: %s firstPhase %d pixelScale ", id, firstPhase);
        for (int i = 0; i < numPreprocs - 1; i++)
    ...
    case 's':
      ...
      // example: ./warpingSIMDTest s living3 day night Hh288sh 0.10 10 4
      {
        ...
        sprintf(gnuplotFile, "%s/homevectors_%s_%d_%d.gp", testPath, id, xssExt,
                yssExt);
        sprintf(gnuplotTrueFile, "%s/truehomevectors_%s_%d_%d.gp", testPath, id,
                xssExt, yssExt);
        ...
        for (int x = 0; x < nx; x++)
          for (int y = 0; y < ny; y++)
            if ((x != xss) || (y != yss)) {
              ...
              warping.run(ss[xss][yss], cv[x][y], interpolation, pixelScale,
                          postScale, alphaTrue, psiTrue, searcher,
                          compassAcceleration, psiFraction,
                          quantilCompassFraction, partialSearch, alphaRad,
                          psiRad, partialFullSearch, doubleSearch, fineSearch,
                          doubleSearchFine, fineSearchSurround, alphaMin,
                          psiMin, dMin, iAlphaMin, iPsiMin, iAlphaMinInv,
                          iPsiMinInv, jAlphaOff, jPsiOff, fineMatch, t1, t2);
    ...
    case 'S':
      ...
      // ./warpingSIMDTest a living3 day night Hh288sh 0.10 gridStep boxRad
      {
              ...
              for (int y = yt; y <= yb; y += gridStep)
                if ((x != xss) || (y != yss)) {
                  ...
                  for (int p = 0; p < pairs; p++) {
                    ...
                    // 27. Feb 18 (rm): vert.res. and hor. taken from images
                    warping.run(
                      _ss_, _cv_, interpolation, pixelScale, postScale,
                      alphaTrue, psiTrue, searcher, compassAcceleration,
                      psiFraction, quantilCompassFraction, partialSearch,
                      alphaRad, psiRad, partialFullSearch, doubleSearch,
                      fineSearch, doubleSearchFine, fineSearchSurround,
                      alphaMin, psiMin, dMin, iAlphaMin, iPsiMin, iAlphaMinInv,
                      iPsiMinInv, jAlphaOff, jPsiOff, fineMatch, t1, t2);
                    // home vector angle (relative to grid x axis!)
                    double betaMin = -alphaMin + psiMin + M_PI - rotAngle[x][y];
                    // errors in [0,pi]
                    ...
                    fprintf(fAll, "%d %d %d %d %g %g %g %g %d\n", xss, yss, x,
                            y, d, betaError, psiError, alphaError, p);
        ...
        printf("RESULTS: betaAE %g %g psiAE %g %g alphaAE %g %g "
               ...
               alphaErrorSum / n, alphaErrorList[underMedianIndex], t1sum / n,
               t2sum / n, n);
#endif
      }
      break;
    ...
    case 'v':
      ...
      // example: ./warpingSIMDTest v living3 day night Hh288sh 0.10 9 4 5 2
      {
        ...
        xcv = xcvExt - xmin;
        ycv = ycvExt - ymin;
        sprintf(fnSuffix, "%s_%d_%d_%d_%d", id, xssExt, yssExt, xcvExt, ycvExt);
        fprintf(finfo, "ss internal (%d, %d)\n", xss, yss);
        ...
        printf("symScaleFac: ");
        for (unsigned i = 0; i < warping.wsc->symmScaleFac.scaleFactors.size();
             i++)
        ...
        // 27. Feb 18 (rm): vert.res. and hor. taken from images
        warping.run(
          ss[xss][yss], cv[xcv][ycv], interpolation, pixelScale, postScale,
          alphaTrue, psiTrue, searcher, compassAcceleration, psiFraction,
          ...
          iAlphaMinInv, iPsiMinInv, jAlphaOff, jPsiOff, fineMatch, t1, t2);
#if 1
        ...
        warping.wsc->minWarping.searchTemplate->getImage(
          minTemplate, MATCHTYPE(255), -20.0, 200.0);
        ...
        fprintf(finfo, "MIN: (iAlphaMin, iPsiMin) = (%d, %d) | (%d, %d)\n",
                iAlphaMinVec[0], iPsiMinVec[0], iAlphaMinVec[1], iPsiMinVec[1]);
        // spsArray of current searcher method
        WarpingSPS<SPSTYPE, SW, SA> **spsArray =
          warping.wsc->getSPSArray(searcher);
        // planes
        for (int spsNo = warping.wsc->SPS_ORIG; spsNo <= warping.wsc->SPS_INV;
             spsNo++) {
          fprintf(finfo, "processing sps %d\n", spsNo);
          // save SPS in binary format
          char spsFile[570];
          sprintf(spsFile, "%s/sps_%d_%s.dat", testPath, spsNo, fnSuffix);
          ...
          case WarpingSearcherSelector::minWarpingSearcher: {
            ...
            minWarpingCurve(*(warping.wsc->minWarping.searchTemplate),
                            *(warping.wsc->minWarping.spsArray[spsNo]),
                            iAlphaMinVec[spsNo], iPsiMinVec[spsNo], jDeltaVec,
                            planeIndexVec, minVec, jYVec, jXVec, jThetaVec,
                            matchSum);
          } break;
          case WarpingSearcherSelector::warpingSearcher: {
            ...
            warpingCurve(*(warping.wsc->warping.searchTemplate),
                         *(warping.wsc->warping.spsArray[spsNo]),
                         iAlphaMinVec[spsNo], iPsiMinVec[spsNo], jDeltaVec,
                         planeIndexVec, iRhoMin, matchSum);
          ...
          default: fprintf(stderr, "invalid searcher %d\n", searcher); exit(-1);
          }
          fprintf(finfo, "warpCurve: match = %s\n",
                  SIMDDecimal<MATCHTYPE>(matchSum).str);
          char solutionFile[575];
          sprintf(solutionFile, "%s/solution_%d_%s.dat", testPath, spsNo,
                  fnSuffix);
          ...
          for (int jTheta = 0; jTheta < w; jTheta++)
            fprintf(fsol, "%3d %3d %3d\n", jTheta, jDeltaVec[jTheta],
                    planeIndexVec[jTheta]);
          fclose(fsol);
          ...
          for (int i = 0; i < spsArray[spsNo]->stack.numPlanes; i++) {
            ...
            for (int spsFormat = 0; spsFormat <= 2; spsFormat++) {
              sprintf(planeFile, "%s/plane_%d_%u_format%d_%s.pgm", testPath,
                      spsNo, i, spsFormat, fnSuffix);
              // fprintf(finfo, "writing plane %d of sps %d to %s\n",
              //         i, spsNo, planeFile);
              spsArray[spsNo]->getPlane(i, spsFormat, plane, -spsMin,
                                        255.0 / (spsMax - spsMin));
              assert(savePGM(planeFile, plane));
              // min-plane
              sprintf(minPlaneFile, "%s/minplane_%d_format%d_%s.pgm", testPath,
                      spsNo, spsFormat, fnSuffix);
              // fprintf(finfo, "writing minplane of sps %d to %s\n",
              // 	 spsNo, minPlaneFile);
              spsArray[spsNo]->getPlaneMinimum(spsFormat, minPlane, -spsMin,
                                               255.0 / (spsMax - spsMin));
              assert(savePGM(minPlaneFile, minPlane));
            }
            // visualize solution
            // only unshuffled!:
            sprintf(planeFile, "%s/plane_solution_%d_%u_%s.pgm", testPath,
                    spsNo, i, fnSuffix);
            // fprintf(finfo,
            //         "writing plane %d of sps %d with solution to %s\n",
            // 	    i, spsNo, planeFile);
            spsArray[spsNo]->getPlaneUnshuffled(i, plane, -spsMin,
                                                255.0 / (spsMax - spsMin));
            for (int jTheta = 0; jTheta < w; jTheta++)
            ...
            assert(savePGM(planeFile, plane));
            // only unshuffled!
            sprintf(minPlaneFile, "%s/minplane_solution_%d_%s.pgm", testPath,
                    spsNo, fnSuffix);
            // fprintf(finfo, "writing minplane of sps %d with solution to
            // %s\n",
            //		  spsNo, minPlaneFile);
        ...
        for (int matchNo = 0; matchNo < warping.wsc->NUM_MATCH; matchNo++) {
          ...
          sprintf(matchFile, "%s/match_%d_%s.pgm", testPath, matchNo, fnSuffix);
          // fprintf(finfo, "writing match %d to %s\n",
          //	  matchNo, matchFile);
          MATCHTYPE matchMin, matchMax;
          warping.wsc->matchArray[matchNo]->extremaExceptInvalid(matchMin,
                                                                 matchMax);
        ...
        for (int spsNo = 0; spsNo < warping.wsc->NUM_SPS; spsNo++) {
          ...
          spsArray[spsNo]->getQuantilCompassImage(quantilCompass,
                                                  quantilCompassFraction);
          sprintf(compassFile, "%s/compass_%d_%s.dat", testPath, spsNo,
                  fnSuffix);
        ...
        if (fineSearch) {
          ...
          sprintf(fineMatchFile, "%s/finematch_%s.dat", testPath, fnSuffix);
          FILE *ffm;
          ...
          fprintf(ffm, "alphaMin %g psiMin %g dMin %g\n", alphaMin, psiMin,
                  dMin);
          fprintf(ffm, "alpha[iAlphaMin] %g psi[iPsiMin] %g dTheta %g\n",
        ...
        warping.spsComp->computeSPSInputDiagram(
          ss[xss][yss], cv[xcv][ycv], warping.wsc->symmScaleFac.scaleFactors,
          interpolation, pixelScale, postScale, *(warping.wsc->spsArray[0]),
          allMax, allMin, allRange, spsInputDiagram);
        fprintf(finfo, "SPS inputs: allMax = %s, allMin = %s, allRange = %s\n",
                SIMDDecimal<MEASTYPE>(allMax).str,
                SIMDDecimal<MEASTYPE>(allMin).str,
                SIMDDecimal<MEASTYPE>(allRange).str);
        char spsInputFile[573];
        sprintf(spsInputFile, "%s/spsinput_%s.pgm", testPath, fnSuffix);
    ...
    case 'l':
      ...
      // adapted from code by Michael Horst
      {
        ...
        FILE *fg;
        double t1, dMin;
        sprintf(gnuplotFile, "%s/dissim_%s_%d_%d.gp", testPath, id, xssExt,
                yssExt);
        assert(fg = fopen(gnuplotFile, "w"));
        // internal snapshot coordinates
        xss = xssExt - xmin;
        ...
        // 27. Feb 18 (rm): vert.res. and hor. taken from images
        warping.compassMatch(ss[xss][yss], cv[xss][yss], interpolation,
                             pixelScale, postScale, dMin, t1);
        fprintf(fg, "%d %d %f\n\n\n", xss, yss, dMin);
        double t1sum = 0;
        int n        = 0;
        for (xcv = 0; xcv < nx; xcv++) {
          for (ycv = 0; ycv < ny; ycv++) {
            // 27. Feb 18 (rm): vert.res. and hor. taken from images
            warping.compassMatch(ss[xss][yss], cv[xcv][ycv], interpolation,
                                 pixelScale, postScale, dMin, t1);
            fprintf(fg, "%d %d %f\n", xcv, ycv, dMin);
            t1sum += t1;
            n++;
    ...
    default: fprintf(stderr, "invalid action %c\n", action); exit(-1);
    }
  } else {
    // action == x
    // -----------------------------------------------------------------------
    ...
    for (int w = simd_img_elems; w < 500; w += simd_img_elems) {
      for (int step = 1; step < 6; step++)
        if (w % step == 0) {
          ...
          bool valid      = true;
          try {
            SIMDImage<IMGTYPE, SW, SA, Panorama> ss(w, 10, Panorama(0.0, 9.0)),
              cv(w, 10, Panorama(0.0, 9.0));
            ...
            WarpingBundle<IMGTYPE, PROCTYPE, MEASTYPE, SPSTYPE, MATCHTYPE,
                          COMPASSTYPE, SW, SA>
              warping(w, nAlpha_nPsi, nAlpha_nPsi, nScalePlanes, maxScaleFactor,
                      maxThresholdMinWarping, rhoMinMinWarping,
                      rhoMaxMinWarping, rhoMinWarping, rhoMaxWarping,
                      nRhoWarping, firstPhase, searchMethodWarping,
            ...
            warping.run(ss, cv,
                        ...
                        /* postScale */ 100,
                        /* alphaEst */ 0.0, /* psiEst */ 0.0, searcher,
                        compassAcceleration, psiFraction,
                        quantilCompassFraction, partialSearch, alphaRad, psiRad,
                        partialFullSearch, doubleSearch, fineSearch,
                        doubleSearchFine, fineSearchSurround, alphaMin, psiMin,
                        dMin, iAlphaMin, iPsiMin, iAlphaMinInv, iPsiMinInv,
                        jAlphaOff, jPsiOff, fineMatch, t1, t2);
